...
class Draw extends JFrame {
	...
	/**
	 * Constructor for the Draw class to visualize A* search results.
	 *
	 * @param en_des   The start and destination points represented as a 2D array.
	 * @param obstacle An array of obstacle objects in the graph.
	 * @param x_length The length of the graph in the x-direction.
	 * @param y_length The length of the graph in the y-direction.
	 * @param array    An ArrayList containing LinkedLists representing:
	 *                 - The optimal path from the start to the destination.
	 *                 - The nodes currently in the open list.
	 *                 - The nodes that have been considered and removed from
	 *                 further consideration.
	 *
	 *                 Note: This constructor initializes the Draw class to
	 *                 visualize A* search results. It sets the
	 *                 size, location, close operation, and visibility of the
	 *                 JFrame. It also sets the graph parameters
	 *                 and the search result data for visualization.
	 */
	public Draw(double en_des[][], Object[] obstacle, double x_length, double y_length,
			ArrayList<LinkedList<Node>> array) {
		setSize(800, 800);
		setLocation(250, 20);
	...
	/**
	 * Overrides the paintComponents method to draw the graph, search results, and
	 * obstacles.
	 *
	 * @param gg The Graphics object used for drawing.
	 */
		...
		Runnable run = new Runnable() {
			public void run() {
				...
				for (int i = 0; i < array.get(0).size() - 1; i++) {
					...
					line_path.setLine(80 + array.get(0).get(i).x * 6, 680 - array.get(0).get(i).y * 6,
							80 + array.get(0).get(i + 1).x * 6, 680 - array.get(0).get(i + 1).y * 6);
					// g2.fill(o_PATH);
					g2.draw(line_path);
				}
				line_path.setLine(80 + array.get(0).get(array.get(0).size() - 1).x * 6,
						680 - array.get(0).get(array.get(0).size() - 1).y * 6, 80 + en_des[0][1] * 6,
						680 - en_des[1][1] * 6);
				// g2.fill(o_PATH);
				g2.draw(line_path);
				...
				for (int i = 0; i < array.get(1).size() - 1; i++) {
					...
					o_PATH.setFrame(80 + (array.get(1).get(i).x - 0.5) * 6, 680 - (array.get(1).get(i).y + 0.5) * 6,
							2 * 0.5 * 6, 2 * 0.5 * 6);
					g2.fill(o_PATH);
				}
				for (int i = 0; i < array.get(2).size() - 1; i++) {
					...
					o_PATH.setFrame(80 + (array.get(2).get(i).x - 0.5) * 6, 680 - (array.get(2).get(i).y + 0.5) * 6,
							2 * 0.5 * 6, 2 * 0.5 * 6);
					g2.fill(o_PATH);
				...
				for (int i = 0; i < obstacle.length; i++) {
					if (obstacle[i].getClass().equals(Rect.class))// 画矩形
					{
						...
						Rectangle2D rect = new Rectangle2D.Double(a.x - a.x_length / 2.0, a.y - a.y_length / 2.0,
								x_length, y_length);
						g2.setStroke(new BasicStroke(2.0f));
					...
					} else if (obstacle[i].getClass().equals(Polygon.class))// 画多边形
					{
						...
						for (int j = 0; j < a.Points[0].length - 1; j++) {
							line_path.setLine(80 + a.Points[0][j] * 6, 680 - a.Points[1][j] * 6,
									80 + a.Points[0][j + 1] * 6, 680 - a.Points[1][j + 1] * 6);
							g2.draw(line_path);
						}
						line_path.setLine(80 + a.Points[0][a.Points[0].length - 1] * 6,
								680 - a.Points[1][a.Points[0].length - 1] * 6, 80 + a.Points[0][0] * 6,
								680 - a.Points[1][0] * 6);
						g2.draw(line_path);
					} else if (obstacle[i].getClass().equals(Circle.class))// 画圆形
					{
						...
						// System.out.println("i ama heer");
						Ellipse2D o = new Ellipse2D.Double(80 + (c.x - c.r) * 6, 680 - (c.y + c.r) * 6, 2 * c.r * 6,
								2 * c.r * 6);
						g2.setStroke(new BasicStroke(4.0f));
						g2.setColor(Color.BLUE);
						g2.fill(o);
					...
					} else if (obstacle[i].getClass().equals(ellipse.class))// 画椭圆
					{
						...
						Ellipse2D o = new Ellipse2D.Double(80 + (c.x - c.a) * 6, 680 - (c.y + c.b) * 6, 2 * c.a * 6,
								2 * c.b * 6);
						g2.setStroke(new BasicStroke(4.0f));
						g2.setColor(Color.BLUE);
						g2.fill(o);
