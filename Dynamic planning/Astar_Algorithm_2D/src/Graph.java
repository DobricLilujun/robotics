...
public class Graph {
	...
	/**
	 * Represents a graph with obstacles, nodes, and other properties.
	 */
	...
	// public Graph(double x0,double y0,double xlength,double ylength,Object[]
	// obst,double en_des[][],boolean isChev)
	// {
	// if(isChev == true)
	// {
	// this.x0 = x0;
	...
	/**
	 * Checks if a node is within the boundaries of the graph.
	 *
	 * @param a     The node to check.
	 * @param graph The graph containing the boundaries.
	 * @return True if the node is within the graph boundaries, false otherwise.
	 */
	public static boolean isInGraph(Node a, Graph graph) {
		if (((a.x <= graph.x0 + graph.xlength) && (a.x >= graph.x0))
				&& ((a.y <= graph.y0 + graph.ylength) && (a.y >= graph.y0))) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Checks if a node is inside a circle.
	 *
	 * @param a The node to check.
	 * @param c The circle.
	...
	/**
	 * Checks if a node is on a given line segment.
	 *
	 * @param a      The node to check.
	 * @param points The points defining the line segment.
	 * @return True if the node is on the line segment, false otherwise.
	 */
	...
	/**
	 * Checks if a node is inside a polygon.
	 *
	 * @param a      The node to check.
	 * @param points The points defining the polygon.
	 * @return True if the node is inside the polygon, false otherwise.
	 */
	public static boolean isInPolygon(Node a, double points[][]) {
		...
		for (int i = 0; i < points[0].length; i++) {
			...
			if (temp_points[0][i + 1] == temp_points[0][i]) {
				...
				double slope = (temp_points[1][i + 1] - temp_points[1][i])
						/ (temp_points[0][i + 1] - temp_points[0][i]);
				if (((temp_points[0][i] < a.x) && (a.x < temp_points[0][i + 1]))
						|| ((temp_points[0][i + 1] < a.x) && (a.x < temp_points[0][i]))) {
