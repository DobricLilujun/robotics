...
public class Main extends Astar {
	...
	public static void main(String[] args) {
		...
		// Define the starting and destination point {x1, x2} and {y1, y2}
		...
		// Define the size of the map {x0 and y0 left bottom point and xlength and
		// ylength}
		double x0 = 0;
		double y0 = 0;
		double xlength = 100;
		...
		// Define obstacles point
		// You can define Circle, ellipse, Segment, Polygon etcs.
		int obstNum = 0;
		...
		// Define the graph map using the predefined object

		Graph graph = new Graph(x0, y0, xlength, ylength, obst, en_des);

		// Start to use the A* algorithm
		Astar sObj = new Astar();
		// test.Astar sObj_t=new test.Astar();
		// test.Graph gr= Astar.Conver_graph(graph);
		// ArrayList<LinkedList<test.Node_cube>> array_Man = new
		// ArrayList<LinkedList<test.Node_cube>>();
		...
		// Three different heristic function to use.
		// Astart_2D: is the basic astart algorithm
		// Astar_2D_Depth: is the second depth priority algorithm
		// Astar_2D_Cheb： is the third uding chebshev distance
