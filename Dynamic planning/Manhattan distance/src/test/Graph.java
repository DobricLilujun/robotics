...
public class Graph {
	...
	public Node members[][] = new Node[xlength][ylength];

	/**
	 * Constructor for the Graph class.
	 *
	 * @param xlength Length of the graph in the x-direction.
	 * @param ylength Length of the graph in the y-direction.
	 * @param obsts   Array representing obstacle positions.
	...
	public Graph(int xlength, int ylength, int obsts[][], int en_des[][]) {
		...
		if (obsts[0].length < xlength * ylength) {
			...
			System.out.println("Obstacles are more than Nodes in the graph.This graph cann't be drawn.");
			System.exit(-1);
		}
		for (int i = 0; i < xlength; i++) {
			for (int j = 0; j < ylength; j++) {
